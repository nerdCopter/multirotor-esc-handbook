...
| **PWM Switching Frequency** | `16kHz`–`128kHz` / Variable | `24kHz`, `48kHz`, `96kHz` | `16kHz`–`128kHz` / By-RPM | `16kHz`–`96kHz` | **Not a simple "lower is safer" dial** (Ryan Harrell, [oKcyXR7Yx64](https://youtu.be/oKcyXR7Yx64) @25:19: he's seen both directions fix the same symptom on different builds). **BLHeli_32 on 5"/3": two real data points in tension, present both.** A rigorous thrust-stand test (Chris Rosser, [6gv0_jTEYZM](https://youtu.be/6gv0_jTEYZM)) found static **24kHz** the best balance of responsiveness and efficiency on his specific test motors, with by-throttle Variable PWM showing little benefit there — but that's one build under bench-test conditions, not a survey of what pilots actually fly. Widespread real-world practice for non-race 5" freestyle leans **static 48kHz** instead, for smoother/quieter output at a small torque cost — reported directly by this KB's own maintainer and, per that report, common among pilots they know. Neither is "the" answer: 24kHz if you want Rosser's tested max punch/efficiency; 48kHz if you want the smoother, more common freestyle default. Race builds are the exception — 24kHz is the much stronger racing case (see §4). For **AM32**, the sourced default leans **24-48kHz Variable/By-RPM** (Chris Rosser, [3SHzyUaypFw](https://youtu.be/3SHzyUaypFw): "definitely use the variable pwm"), consistent with the creator's own framing of that feature's intended purpose. For whoops on Bluejay, the real tradeoff is **duty-cycle resolution, not desync risk** — Bluejay's own maintainers documented 96kHz's reduced resolution (256 steps, worse with higher ESC dead time) as their reason for almost removing it, then restored it after community pushback — see [bluejay.md](bluejay.md#pwm-switching-frequency-breakdown) for the full sourced history. Primarily a torque/efficiency/resolution/braking tradeoff, not a guaranteed desync fix — see [PWM Frequency & Switching Guide](pwm-frequency-heat.md). |
...
* **Desync Prevention Setup:**
  ...
  * **PWM Frequency — two real data points in tension for BLHeli_32, present both rather than picking one:**
      ...
      * **48kHz:** the more common static choice for non-race 5" freestyle in wider real-world practice — smoother/quieter, small torque cost versus 24kHz. Not from a formal test; a maintainer-reported real-world default.
      ...
      * **AM32** is a separate case with its own sourced default: **24-48kHz Variable/By-RPM** (Chris Rosser, [3SHzyUaypFw](https://youtu.be/3SHzyUaypFw): "definitely use the variable pwm") — consistent with the AM32 creator's own framing of By-RPM as the intended design ([am32.md](am32.md#dynamic-pwm-modes--pwm-by-rpm)). An earlier draft of this guide wrongly generalized that AM32 finding to BLHeli_32 as well — corrected once BLHeli_32-specific data existed, then further balanced against real-world 48kHz practice above rather than swinging to a single opposite answer.
