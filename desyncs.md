...
| **PWM Switching Frequency** | `16kHz`–`128kHz` / Variable | `24kHz`, `48kHz`, `96kHz` | `16kHz`–`128kHz` / By-RPM | `16kHz`–`96kHz` | **Not a simple "lower is safer" dial** (Ryan Harrell, [oKcyXR7Yx64](https://youtu.be/oKcyXR7Yx64) @25:19: he's seen both directions fix the same symptom on different builds). For 5"+, **24kHz** is the sourced choice for maximum braking torque (Joshua Bardwell, [yEDhnBUFQNI](https://youtu.be/yEDhnBUFQNI)). For whoops on Bluejay, the real tradeoff is **duty-cycle resolution, not desync risk** — Bluejay's own maintainers documented 96kHz's reduced resolution (256 steps, worse with higher ESC dead time) as their reason for almost removing it, then restored it after community pushback — see [bluejay.md](bluejay.md#pwm-switching-frequency-breakdown) for the full sourced history. Primarily a torque/efficiency/resolution/braking tradeoff, not a guaranteed desync fix — see [PWM Frequency & Switching Guide](pwm-frequency-heat.md). |
...
* **Desync Prevention Setup:**
  ...
  * **PWM Frequency:** **48kHz** — sourced as the ~3" recommendation (Joshua Bardwell, [yEDhnBUFQNI](https://youtu.be/yEDhnBUFQNI)); a torque/efficiency middle ground, not a desync-risk dial.
...
* **Desync Prevention Setup:**
  ...
  * **PWM Frequency:** Sourced guidance is **24kHz** for 5"+ builds, prioritizing active braking torque (Joshua Bardwell, [yEDhnBUFQNI](https://youtu.be/yEDhnBUFQNI)). **24kHz–48kHz Variable** or **PWM By-RPM** in AM32 are reasonable alternatives if you want more efficiency and are willing to trade some braking authority.
...
* **Desync Prevention Setup:**
  ...
  * **PWM Frequency:** Sourced guidance is **24kHz** for 5"+ builds generally (Joshua Bardwell, [yEDhnBUFQNI](https://youtu.be/yEDhnBUFQNI)), for maximum active-braking torque — even more relevant on heavy lifters.
