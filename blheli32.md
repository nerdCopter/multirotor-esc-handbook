...
* **Legacy Variable PWM (by throttle):** continuously increases switching frequency with throttle stick position (24kHz at low throttle for braking/low-end torque, up to the configured high-throttle bound — target-dependent, up to the firmware's 128kHz ceiling). Sourced failure mode (Joshua Bardwell, [xuQeJA4EGr8](https://youtu.be/xuQeJA4EGr8)): fixed-by-throttle PWM can interact with actual motor RPM to create mid-throttle mechanical/electrical resonance ("jello"/harmonics) at specific throttle bands.
* **"By RPM" PWM:** added in **BLHeli_32 firmware v32.8.3** (sourced, [xuQeJA4EGr8](https://youtu.be/xuQeJA4EGr8)) — switches frequency based on actual measured motor RPM instead of throttle position, which is what fixes the resonance issue above. See [PWM Frequency & Switching Guide](pwm-frequency-heat.md). **Real sourced range** (Chris Rosser, [6gv0_jTEYZM](https://youtu.be/6gv0_jTEYZM)): rises only modestly, roughly **24kHz up to ~40kHz max** — stays within the low-frequency range his testing found best for braking/efficiency. Same source, on Fixed vs. By-RPM: "I always run 24k fixed pwm," suggesting By-RPM only if you specifically notice throttle notchiness — not a strict upgrade.
* **Critical, size-specific PWM limit** (same source): on a 3" motor (12-pole/9-coil), fixed PWM above **48kHz** caused the motor to fail to accelerate cleanly at all — audibly "jerking," sometimes failing to spin up, with no usable data above that point. A 5" motor (14-pole/12-coil) didn't show this specific failure, only reduced acceleration at 128kHz. **24kHz tested as the sweet spot for both 5" and 3"** motors — best balance of responsiveness and efficiency; braking also degrades monotonically as frequency rises on both sizes.
